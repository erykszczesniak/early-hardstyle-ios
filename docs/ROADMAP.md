...
> Planned work, in priority order. Each item ships as its own branch + PR per `the project brief`.
...
| 2.3 | **Search screen** per `the design spec` §4.7 — full-screen cover, grouped results (Sets / DJs / Events), recent searches as chips | Closes the last designed-but-unbuilt screen from the spec. | M |
...
| 2.8 | **Metal audio-reactive visualizer** behind the player artwork | The `the design spec` §3.8 extended-scope item — pure portfolio flex (Shaders/Metal). | L |
